...
//    if (tmpBuffer[0] == DATA_FrameHead && tmpBuffer[1] == DATA_FrameHead) {
    if (tmpBuffer[0] == DATA_FrameHead_1 && tmpBuffer[1] == DATA_FrameHead_2) {
        switch (tmpBuffer[2]) {  // check tmpBuffer[2] is Q4 data or not. for diff. imu, data is diff.
            case DATA_TYPE_Q4:  //该帧输出数据为四元数类型
            {
                Q4[0] = (double) (tmpBuffer[4] << 8 | tmpBuffer[5]) / Q4_MEASURE;  // tmpBuffer[4] binary data left shift 8 bits, then use op. or with tmpBuffer[5]
            ...
            case DATA_TYPE_MAG: {
                std::cout << "未更新DATA_TYPE_MAG解码程序！" << std::endl;
                break;
            }   //该帧输出数据为磁力计类型
            ...
            case DATA_TYPE_STOP: {
                std::cout << "未更新DATA_TYPE_STOP解码程序！" << std::endl;
                break;
            } //该帧输出数据为保留不用类型

            case DATA_TYPE_PRECISION: {
                std::cout << "未更新DATA_TYPE_PRECISION解码程序！" << std::endl;
                break;
            }  //该帧输出数据为传感器精度，频率类型

            case DATA_TYPE_RANGE: {
                std::cout << "未更新DATA_TYPE_RANGE解码程序！" << std::endl;
                break;
            }  //该帧输出数据为传感器量程
            default:
                break;
...
bool IPSG::CImuCommand::cmdFrame(unsigned char imucmd) {
    ...
    cmd_buffer[1] = imucmd;
    cmd_buffer[2] = CMD_FrameHead + cmd_buffer[1];  // checksum for each piece of data: |head|imucmd|checksum|
    //return commd_buffer;

    ser.write(cmd_buffer, cmd_num );  // write a string into serial port

    ser.read(r_buffer, READ_BUFFERSIZE);  // read from serial into given buffer, org. SIZE 13 bytes
    ...
    decodeFrame(r_buffer);  // decode only for r_buffer, original 13 bytes, change to 23 bytes
    return true;
}
